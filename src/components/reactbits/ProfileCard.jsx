...
export default function ProfileCard({ member }) {
  return (
    ...
    >
      ...
      <motion.div 
        className="absolute bottom-0 left-0 w-full p-6 z-20 flex flex-col justify-end translate-y-4 group-hover:translate-y-0 transition-transform duration-300"
      >
        <h3 className="text-white text-xl font-bold mb-1 tracking-tight">{member.name}</h3>
        {member.role && <p className="text-blue-400 text-xs font-bold uppercase tracking-widest mb-3">{member.role}</p>}
        
        <div className="opacity-0 group-hover:opacity-100 transition-opacity duration-300 delay-100">
          {member.instagram ? (
            <a href={member.instagram} target="_blank" rel="noreferrer" className="inline-flex items-center gap-2 bg-white/20 backdrop-blur-md px-4 py-2 rounded-full text-white text-sm font-medium hover:bg-white hover:text-black transition-colors">
              <Instagram size={16} /> @instagram
            </a>
          ) : (
            <span className="text-white/60 text-xs font-medium bg-black/40 px-3 py-1.5 rounded-full backdrop-blur-md">No Social</span>
          )}
        </div>
      </motion.div>
    </motion.div>
